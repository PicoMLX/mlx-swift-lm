...
struct PromptCacheTests {
    ...
    @Test("Disk round trip keeps salt namespaces separate")
    func diskRoundTripKeepsSaltNamespacesSeparate() async throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("LRUPromptCacheTests-\(UUID().uuidString)", isDirectory: true)
        defer {
            try? FileManager.default.removeItem(at: directory)
        ...
        // The persisted entry only resurfaces under its original salt.
        let matching = restored.fetchNearestCacheResult(model: "model", tokens: [1, 2, 3], salt: 1)
        #expect(matching.hitKind == .exact)

        let mismatched = restored.fetchNearestCacheResult(
            model: "model", tokens: [1, 2, 3], salt: 2)
        #expect(mismatched.cache == nil)
        #expect(mismatched.hitKind == .none)
    }
    ...
    @Test("Rotating-only entries are skipped by disk persistence")
    func rotatingOnlyEntriesAreSkippedOnDisk() async throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("LRUPromptCacheTests-\(UUID().uuidString)", isDirectory: true)
        defer {
            try? FileManager.default.removeItem(at: directory)
        ...
        cache.insertCache(
            model: "model",
            tokens: [1, 2, 3],
            promptCache: [makeRotatingCache(seqLen: 3, maxSize: 64)]
        )
        try await cache.save(to: directory, maxDiskBytes: nil)

        // Rotating caches are not disk-persistable, so nothing is restored.
        let restored = LRUPromptCache(maxSize: 10)
        try await restored.load(from: directory, allowedModels: ["model"])
    ...
    @Test("LRUPromptCache is reachable as a PersistablePromptCache")
    func conformsToPersistablePromptCache() async throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("LRUPromptCacheTests-\(UUID().uuidString)", isDirectory: true)
        defer {
            try? FileManager.default.removeItem(at: directory)
        ...
        let cache: any PersistablePromptCache = LRUPromptCache(maxSize: 10)
        cache.insertCache(
            model: "model",
            tokens: [1, 2, 3],
            promptCache: makeSimplePromptCache(seqLen: 3),
        ...
        let restored: any PersistablePromptCache = LRUPromptCache(maxSize: 10)
        try await restored.load(from: directory, allowedModels: nil)

        let hit = restored.fetchNearestCacheResult(model: "model", tokens: [1, 2, 3], salt: 0)
        #expect(hit.hitKind == .exact)
    }
}
