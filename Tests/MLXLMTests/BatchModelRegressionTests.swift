...
/// Numerical regression: batched prefill/decode must match single-stream,
/// token-for-token, under greedy decoding. These catch mask / RoPE / padding
/// bugs that do not surface as compile errors.
///
/// No Metal-availability guard is used (matching the PR1 cache tests); the
/// real gate is the macOS / Metal reviewer + CI, which runs these end to end.
@Suite("Batch model regressions", .serialized)
