...
public final class LRUPromptCache: PromptCaching {
    ...
    deinit {
        // Tear the trie down iteratively. A stored prompt is a
        // token-per-node chain of class instances, so letting ARC release a
        // root recursively (node → children → node → …) needs stack
        // proportional to the longest stored prompt — thousands of frames
        // for a long context, enough to overflow the 512 KB stack of a
        // cooperative-pool thread (a stack overflow kills the process with
        // no diagnostic at all). Detaching children level by level frees
        // every node with bounded stack. Entry removal elsewhere is already
        // iterative: `delete` prunes empty nodes one at a time, bottom-up.
        var pending = state.withLockUnchecked { state -> [TrieNode] in
            let roots = Array(state.cache.values)
